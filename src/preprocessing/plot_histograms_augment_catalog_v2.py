...
# Function to extract data from a .h5 file using preprocessed data
def extract_data_from_h5(file, default_total_charge=None):
    """Extract data from .h5 file."""
    try:
        with h5py.File(file, "r") as f:
            def safe_get(key, default=float('NaN')):
                """Safely retrieve a value from HDF5."""
                if key in f:
                    data = f[key]
                    if data.shape == ():  # Scalar dataspace
                        return data[()]
                    return data[:]
                return default

            # Extract values
            norm_emit_x = safe_get('norm_emit_x')
            norm_emit_y = safe_get('norm_emit_y')
            norm_emit_z = safe_get('norm_emit_z')

            settings = {
                'SOL10111:solenoid_field_scale': safe_get('SOL10111_solenoid_field_scale'),
                'CQ10121:b1_gradient': safe_get('CQ10121_b1_gradient'),
                'SQ10122:b1_gradient': safe_get('SQ10122_b1_gradient'),
                'GUNF:rf_field_scale': safe_get('GUNF_rf_field_scale'),
                'GUNF:theta0_deg': safe_get('GUNF_theta0_deg'),
                'distgen:total_charge': safe_get('distgen_total_charge', default_total_charge),
            }
