...
class StepPairGraphDataset(Dataset):
    def __init__(self, graph_data_dir, initial_step, final_step, task='predict_n6d', use_settings=False,
        ...
        # Preload data if enabled
        if self.preload_data:
            self.initial_graphs = [torch.load(f) for f in self.initial_graph_files]
            self.final_graphs = [torch.load(f) for f in self.final_graph_files]
            if self.use_settings and not self.identical_settings:
                self.settings = [torch.load(f) for f in self.settings_files]
        else:
            self.initial_graphs = None
            self.final_graphs = None
            if self.use_settings and not self.identical_settings:
    ...
    def __getitem__(self, idx):
        ...
        if self.use_settings:
            if self.identical_settings:
                settings_expanded = settings.unsqueeze(0).expand(initial_graph.num_nodes, -1)
            else:
                settings_expanded = settings.unsqueeze(0).expand(initial_graph.num_nodes, -1)
            initial_graph.x = torch.cat([initial_graph.x, settings_expanded], dim=1)
        ...
        # Compute edge attributes manually if required
        if self.use_edge_attr:
            self._compute_edge_attr(initial_graph)
