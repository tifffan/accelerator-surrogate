...
class GraphPredictionTrainer(BaseTrainer):
    ...
    def model_forward(self, data):
        ...
        if model_type == 'GeneralGraphNetwork':
            x_pred, edge_attr_pred, global_pred = self.model(
                x=data.x,
                edge_index=data.edge_index,
                edge_attr=data.edge_attr,
                u=data.set.unsqueeze(0),  # Expand dimensions to match batch size
                batch=data.batch
            )
            ...
            x_pred = self.model(
                ...
                edge_index=data.edge_index,
                edge_attr=data.edge_attr,
                conditions=data.set.unsqueeze(0),  # Expand dimensions
                batch=data.batch
            )
