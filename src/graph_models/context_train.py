...
if __name__ == "__main__":
    ...
    data_loaders = GraphSettingsDataLoaders(
        ...
        task=args.task,
        use_edge_attr=use_edge_attr,
        edge_attr_method=args.edge_attr_method if hasattr(args, 'edge_attr_method') else 'v0',  # Default to 'v0' if not specified
        preload_data=args.preload_data if hasattr(args, 'preload_data') else False,          # Default to False if not specified
